...
class FunctionCache {
    ...
    setOption(name, value, init) {
        ...
        const required_type =
            name.indexOf('callback') !== -1 ? 'function' : 'string'
    ...
    async get(func, args, ttl, no_freeze, callback) {
        ...
        const key2 = args.length
            ? farmhash.hash32(stringifyFlatted(args))
            : 'empty'

        if (
            this.data[key1] !== undefined &&
            this.data[key1][key2] !== undefined
        )
            return Promise.resolve(this.data[key1][key2].value)
        let result = func(...args)
        if (result instanceof Promise) result = await result

        if (this.set_callback)
            this.set_callback(this.set_event_name, [
                key1,
                key2,
                result,
                ttl || 0,
    ...
    delete(k, args, dont_clear_timeout, callback) {
        ...
        const key1 =
            _type !== 'string' && _type !== 'number'
                ? _type === 'function'
                    ? farmhash.hash32(k.toString())
                    : farmhash.hash32(stringifyFlatted(k))
                : k
        ...
        if (this.delete_callback && (key2 || !dont_clear_timeout))
            this.delete_callback(this.delete_event_name, [key1, key2])
